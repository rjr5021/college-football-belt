...
resolve_vacancies() during workflow run #76, confirmed to exist identically
in build_losers_lineage.py's own resolve_vacancies() (this file forked
belt_engine.py's has_gap forgiveness loop -- or rather belt_engine.py forked
this one -- and both carried the same flaw). This is a THIRD, distinct bug
from the two chain_teams cycle bugs already fixed (see test_losers_cycle_fix.py
and test_losers_cycle_fix_terminal.py): it needs no cycle among multiple
teams at all, just a single origin reign with more than one un-revertable
gap before real predecessor data resumes.
...
that was never counted. walk_losers() is a pure function of its arguments,
...
arguments and got the identical result back -- forever. Caught live via
belt_engine.py when the SIAA conference's from-scratch bootstrap walk hit
exactly this shape; this file's own resolve_vacancies() was never actually
seen looping live (no losers-belt scope happens to have this exact shape
yet), but code inspection confirmed the identical structural flaw, so it
gets the identical fix and the identical regression test.

This builds a minimal fixture where the origin holder (A) has TWO separate
over-threshold silences before finally losing the LOSERS belt for real
(i.e. winning a game, handing the belt to whoever it just beat), and checks
the walk both terminates and produces the correct 2-reign, zero-vacancy
result. A 15s SIGALRM safety net guards the test itself.
"""
import signal
import sys
...
games = [
    ...
    game(2, d(600), "A", "Y", 0, 10),     # A loses to Y -> real defense, if forgiven
    ...
    game(3, d(1300), "A", "Z", 0, 10),    # A loses to Z -> real defense, if forgiven
    # A finally loses the LOSERS BELT for real by WINNING outright,
    # immediately after -- no gap here. B (the team A just beat) catches it.
    game(4, d(1301), "A", "B", 10, 0),    # A beats B -> B catches (won_from=A)
]
...
assert [r["team"] for r in reigns] == ["A", "B"], (
    f"Expected exactly two reigns (A, then B after A really lost the belt "
    f"by winning outright), got {[r['team'] for r in reigns]}")
...
assert a_reign["end_date"] == d(1301) and a_reign["lost_to"] == "B", (
    "Expected A's reign to close out by really losing the belt to B, not "
    f"by being reverted -- got end_date={a_reign.get('end_date')}, "
    f"lost_to={a_reign.get('lost_to')}")

print("\nALL LOSERS-BELT MULTI-GAP FIX TESTS PASSED")

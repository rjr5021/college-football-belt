...
(with a link to browse the rest) only as a short footer.
"""
...
def compose_otd_tweet(game, belt_games, today, extra_count):
    ...
    if extra_count:
        lines.append(f"That's not even the only one -- {extra_count} more belt game{'s' if extra_count != 1 else ''} "
                      f"happened on this date. See them all \U0001F447")
        lines.append(f"{SITE_URL}/on-this-day.html")
    else:
        lines.append(f"Full game \U0001F447\n{game_url}")
    lines.append("")
    lines.append("#CFB")
...
def main():
    ...
    text = compose_otd_tweet(headline, belt_games, today, len(matches) - 1)
